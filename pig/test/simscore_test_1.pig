register ../libs/ginko.jar;
define SIMSCORE io.enjapan.il.ginko.CompanySimilarity;

lbc = LOAD '../data/test/lbc.csv' USING PigStorage(',') as(office_id:long,company_name:chararray,
    company_name_kana:chararray,representative:chararray,representative_kana:chararray,
    company_tel:chararray,company_vitality:chararray,ind1:int,ind2:int,ind3:int,
...
lbc = FOREACH lbc GENERATE
    ...
    (long)ROUND((RANDOM()*10.0)) as id2:long;
SPLIT lbc INTO X if id2 > 5L, Y IF id2 <=5L;
Y2 = FOREACH Y GENERATE *, id2+4L as id3:long;
...
STORE scored INTO '../data/test/output/scored' USING PigStorage (',');
